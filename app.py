...
# --- Load Stripe keys and App URL with default ---
try:
    ...
    st.error(
        f"⚠️ Missing required secret: `{e.args[0]}`.\n"
        "Add `stripe.secret_key`, `stripe.publishable_key`, and optionally `[app].url` to `.streamlit/secrets.toml`."
    )
    secrets_ok = False
...
_Set your Stripe keys and app URL via `.streamlit/secrets.toml`._
""" )
